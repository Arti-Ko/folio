...
final class Updater {
    ...
    /// Выпуск, о котором нужно спросить в окне обновления.
    private(set) var presentedRelease: ReleaseInfo?
    ...
    @ObservationIgnored private let logger = Logger(subsystem: "org.sleepycoffee.folio", category: "updates")
    ...
    func present(_ release: ReleaseInfo) {
    ...
    func check(userInitiated: Bool) async {
        if case .readyToInstall(let release) = phase {
            if userInitiated { present(release) }
            return
        }
        ...
        do {
            ...
            guard let release, let currentVersion, release.version > currentVersion else {
                ...
                if userInitiated {
                    showAlert(
                        ...
                        message: "Folio \(currentVersion?.description ?? "") — самая свежая версия."
                    )
                }
                return
            }
            ...
            if !userInitiated, installsAutomatically, UpdateInstaller.canReplace(bundleAt: Bundle.main.bundleURL) {
            ...
            logger.error("Проверка обновлений: \(error.localizedDescription, privacy: .public)")
            if userInitiated {
    ...
    func installPendingUpdateOnQuit() {
        ...
        do {
            ...
            logger.error("Установка при выходе: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func download(_ release: ReleaseInfo, relaunchWhenReady: Bool) {
        ...
        downloadTask = Task { [weak self] in
            do {
                let app = try await UpdateInstaller.download(release, expectedBundleID: bundleID) { fraction in
                    Task { @MainActor in
                ...
                self?.logger.error("Загрузка обновления: \(error.localizedDescription, privacy: .public)")
            }
